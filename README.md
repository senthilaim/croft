...
sets `--bes_backend=grpc://localhost:9095` plus two `--bes_header`s, `x-workspace-id=<id>` and
`x-workspace-token=<hmac>` (a per-workspace token the gRPC server verifies before accepting
anything, computed as `HMAC-SHA256(BES_INGEST_SECRET, workspaceId)` so nothing needs to be
generated or stored per workspace), so Bazel connects directly and streams each
`build_event_stream.BuildEvent` (wrapped in a `google.protobuf.Any`) over that authenticated
connection as the build progresses.
...
Known limits: the Buildfarm remote-cache/executor endpoint has no authentication yet (the
build-event endpoint does -- see "Known simplifications" below), so keep it on a private network or
VPN (the page warns about this, and about `localhost` and GitHub-hosted runners, which cannot reach
a private Croft).
...
with the access token attached as a Bearer header. On a `401` (the access token expired), that same
route silently calls `POST /auth/refresh` with the refresh-token cookie, retries the original
request once with the new access token, and rotates both cookies (refresh rotates the refresh token
too) -- a normal session never gets logged out mid-use just because 15 minutes passed. If the
refresh token itself is also invalid/expired, the original `401` passes through unchanged and
`proxy.ts` (Next's renamed middleware) optimistically redirects unauthenticated requests to
`/workspaces/*` back to `/signin`.
...
- **The Buildfarm's own remote-cache/executor port has no authentication** — every other
  sandboxed job and the BES ingest port (see "Build analytics" above) are authenticated; this one
  genuinely isn't, because bazel-buildfarm itself (the pinned image) has no auth story beyond
  server-side TLS (encryption only, no client auth). A real fix needs an auth-terminating proxy in
  front of the existing plaintext port — a new feature, not a quick patch, and binding the port to
  loopback instead would break the "Connect an existing repo" CI feature, whose entire point is
  letting a remote CI runner reach it. Keep this port on a private network or VPN; do not expose it
  to the public internet. Tracked as a deferred, deliberate gap, not an oversight.
