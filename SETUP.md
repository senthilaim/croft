...
11. **Explore with a demo run** — before connecting anything, open **Analytics** and click **Run demo**
   (no running Buildfarm needed). It generates about 40 sample builds over two weeks — a warming
   remote cache, four kinds of failure with located fixes, and a flaky test — so the dashboard,
   Failures/Targets/Remote cache tabs, Test grid and Trends all have data. Everything is marked
...
see; if a row fails, use the "If it fails" pointer and don't continue until it passes. Use a
workspace's sample project (step 4.5) for the build steps -- the same checks apply to any project
you connect. In Docker mode replace `localhost:9000` checks with `docker compose ps`.

| # | Feature | Do this | Pass when |
|---|---|---|---|
| 1 | Platform health | `curl -s localhost:4000/api/health` (dev) or `docker compose ps` (Docker) | Backend reports Mongo connected; all services `healthy`/`Up` |
